...
You can download and save this dataset wherever you like, and just pass that filepath into `theft::process_hctsa_file` which automatically processes the Matlab file into a nice tidy dataframe. I just have the dataset in my downloads (saved as "INP_Bonn_EEG.mat"), so I can run the following:

```{r, include = FALSE, warning = FALSE, error = FALSE}
tmp <- process_hctsa_file("/Users/trenthenderson/Downloads/INP_Bonn_EEG.mat") %>%
  mutate(id = unlist(id),
         group = unlist(group))
```
